...
def logout(request):
	try:
		...
		msg2="User Logged Out Sucessfully"
		return render(request,'login.html',{'msg2':msg2})
	except:
		return render(request,'login.html')
...
def forgot_password(request):
	if request.method=="POST":
		try:
			...
			return render(request,'opt.html',{'email':user.email,'otp':otp})
		except:
...
def verify_otp(request):
	...
	if otp==uotp:
		...
		msg="Invalid OTP"
		return render(request,'opt.html',{'email':email,'otp':otp,'msg':msg})
...
def seller_edit_product(request,pk):
	...
	if request.method=="POST":
		...
		try:
			...
			pass
		product.save()
		msg="Product Edited Sucessfully"
